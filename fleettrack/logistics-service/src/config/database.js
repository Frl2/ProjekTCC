...
const pool = mysql.createPool({ ...poolConfig, database: process.env.DB_NAME || 'logistics_db' });
...
async function initDB() {
  ...
  try {
    ...
    await conn.query('CREATE DATABASE IF NOT EXISTS logistics_db');
    await conn.query('USE logistics_db');

    const schemaPath = path.join(__dirname, '../../database/logistics_schema.sql');
    const seedPath = path.join(__dirname, '../../database/logistics_seed.sql');
    ...
    console.log('✅ Database logistics_db initialized');
  } catch (err) {
